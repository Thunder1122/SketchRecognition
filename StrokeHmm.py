...
class HMM:
    ...
    def label(self, data) :
        ...
        information = [] 
        # Formatted as [@time 0: {state1: {probability: p, prev_state: state}
        #                       state2: {probability: p, prev_state: state1},
        #                       ...}
        #              {@time 1: ...}]
        for timestep, evidence in enumerate(data):
            statedict = {}
            for state in self.states:
                statedict[state] = {}
                if timestep == 0:
                    prior_prob = math.log(self.priors[state])
                    evidence_prob = math.log(self.emissions[state][evidence])
                    prob = prior_prob + evidence_prob
                    statedict[state]['probability'] = math.e ** prob
                    statedict[state]['prev_state'] = None
                else:
                    max_prob = float('-inf')
                    max_state = None
                    evidence_prob = math.log(self.emissions[state][evidence])
                    for prev_state in self.states:
                        prior_prob = math.log(information[timestep-1][prev_state]['probability'])
                        transition_prob = math.log(self.transitions[prev_state][state])
                        prob = prior_prob + evidence_prob + transition_prob
                        if prob > max_prob:
                            max_prob = prob
                            max_state = prev_state
                    statedict[state]['probability'] = math.e ** max_prob
                    statedict[state]['prev_state'] = max_state

            information.append(statedict)

        # Find a starting point to trace back through the states
        information.reverse()
        final_path = []
        max_state = None
        max_prob = 0
        for state in information[0].keys():
            cur_prob = information[0][state]['probability']
            if cur_prob > max_prob:
                max_prob = cur_prob
                max_state = state

        # Trace back through the states
        final_path.append(max_state)
        for i in range(len(information) - 1):
            next_state = information[i][final_path[-1]]['prev_state']
